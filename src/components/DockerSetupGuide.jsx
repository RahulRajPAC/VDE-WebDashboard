...
export default function DockerSetupGuide({ status, onRetry }) {
    ...
    const isMissing = status.installed === false;
    const isDown = status.installed === true && status.running === false;
                ...
                <CardContent className="space-y-6 px-8">
                    ...
                    {isDown && (
                        <div className="space-y-4">
                            ...
                            {status.error === 'PAUSED' && (
                                <div className="rounded-lg bg-muted/50 p-6 border flex items-start gap-4">
                                    ...
                                    <div className="text-left space-y-1 w-full">
                                        <h3 className="font-semibold text-lg">Docker is Paused</h3>
                                        <p className="text-muted-foreground leading-relaxed">
                                            We detected the Docker CLI and Engine, but it is currently <strong>manually paused</strong>.
                                            Please open the Docker Desktop application (or click the Whale icon in your menu bar) and unpause the engine to continue.
                                        </p>
                                    </div>
                                </div>
                            )}

                            {/* Standard Offline or Frozen State */}
                            {status.error !== 'PAUSED' && (
                                <div className="rounded-lg bg-muted/50 p-6 border flex items-start gap-4">
                                    <div className="mt-1 bg-yellow-100 dark:bg-yellow-900/20 p-2 rounded-full">
                                        <Terminal className="w-6 h-6 text-yellow-600 dark:text-yellow-500" />
                                    ...
                                    <div className="text-left space-y-1 w-full">
                                        <h3 className="font-semibold text-lg">
                                            {status.error === 'TIMEOUT' ? 'Is Docker Frozen?' : 'Start Docker Desktop'}
                                        </h3>
                                        <p className="text-muted-foreground leading-relaxed">
                                            We detected the Docker CLI, but the engine is unresponsive.
                                            {status.error === 'TIMEOUT'
                                                ? " It looks like Docker Desktop might be Frozen. Please open the application or restart it to ensure it is running properly."
                                                : " Please open the Docker Desktop application on your computer and wait for it to fully initialize."}
                                        </p>
                                        <Button
                                            variant="outline"
                            ...
                            {status.error && status.error !== 'TIMEOUT' && status.error !== 'PAUSED' && (
